...
struct MaxOp
{
    CUTLASS_DEVICE T operator()(T const& x, T const& y) { return x > y ? x : y; }
};

template <>
struct MaxOp<float>
{
    CUTLASS_DEVICE float operator()(float const& x, float const& y) { return max(x, y); }
};

template <typename T>
struct SumOp
{
    CUTLASS_DEVICE T operator()(T const& x, T const& y) { return x + y; }
};
...
struct ShflReduce
{
    ...
    static CUTLASS_DEVICE T run(T x, Operator& op)
    {
...
CUTLASS_DEVICE static void applySoftmaxNormalizer(const Fragment0& sPrime, Fragment1& accum)
{
...
CUTLASS_DEVICE static void onlineSoftmaxAndRescale(Fragment0& mi, Fragment1& sPrime, Fragment2& accum, Fragment3& accum_o,
                                                   float scaleFactor)
{
    using namespace cute;
    using FragValType = typename Fragment2::value_type;
    using FragValType0 = typename Fragment3::value_type;

    auto landId = cutlass::canonical_lane_idx();

    // First update mi to max per-row
...
#pragma unroll
        ...
        if (!isFirst)
        {
            auto m_prime_exp0 = exp2(miPrev(rowId) - max_quad_0);
            sPrime(rowId) *= m_prime_exp0;

            auto m_prime_exp1 = exp2(miPrev(rowId + 1) - max_quad_1);
            sPrime(rowId + 1) *= m_prime_exp1;

            for (int k = 0; k < size(shape<2>(accum_o)) * size<2>(shape<0>(accum_o)); ++k)
            {
                data_o[no] = FragValType0(AccumType(data_o[no]) * m_prime_exp0);
                ++no;

                data_o[no] = FragValType0(AccumType(data_o[no]) * m_prime_exp0);
                ++no;

                data_o[no] = FragValType0(AccumType(data_o[no]) * m_prime_exp1);
                ++no;
