...
__global__ void GemmReferenceKernel(const int M, const int N, const int K,
                                    float alpha, const T* A, const T* B,
                                    float beta, TC* C)
{
...
template <typename T, typename TC>
void GemmReference(const int M, const int N, const int K, float alpha,
                   const T* A, const T* B, float beta, TC* C)
{
    dim3 block(16, 16);
...
template <class OperatorClass, class ArchTag, class ElementA, class LayoutA,
          class ElementB, class LayoutB, class ElementC, class LayoutC,
          class ElementAccumulator>
struct GemmConfiguration
{
    static_assert(sizeof(ElementA) == 0,
                  "No valid GemmConfiguration configuration exists.");
};
...
template <>
struct Gemm_TensorOpSm80_OperandA<cutlass::half_t, cutlass::layout::RowMajor, 8,
                                  64>
{
    // Smem
    ...
    using GmemTiledCopy = decltype(make_tiled_copy(
        Copy_Atom<SM80_CP_ASYNC_CACHEALWAYS<cute::uint128_t>,
                  cutlass::half_t>(),
...
template <int SizeK>
struct Gemm_TensorOpSm80_OperandA<cutlass::half_t, cutlass::layout::ColumnMajor,
                                  8, SizeK>
{
    // Smem
    using SmemLayoutAtom = decltype(composition(
...
struct Gemm_TensorOpSm80_OperandB<cutlass::half_t, cutlass::layout::ColumnMajor,
                                  Alignment, SizeK>
    : Gemm_TensorOpSm80_OperandA<cutlass::half_t, cutlass::layout::RowMajor,
                                 Alignment, SizeK>
{
};
...
template <int Alignment, int SizeK>
struct Gemm_TensorOpSm80_OperandB<cutlass::half_t, cutlass::layout::RowMajor,
                                  Alignment, SizeK>
    : Gemm_TensorOpSm80_OperandA<cutlass::half_t, cutlass::layout::ColumnMajor,
                                 Alignment, SizeK>
{
};
                         ...
                         float, LayoutC, float>
{
    ...
    using TiledMma = TiledMMA<MMA_Atom<SM80_16x8x16_F32F16F16F32_TN>,
                              ...
                              Tile<_32, _32, _16> // 32x32x16 MMA for LDSM,
                                                  // 1x2x1 value group`
                              >;

    // A
    using OperandA =
        Gemm_TensorOpSm80_OperandA<ElementA, LayoutA, AlignmentA, 32>;
    ...
    // B
    using OperandB =
        Gemm_TensorOpSm80_OperandB<ElementB, LayoutB, AlignmentB, 32>;
    ...
    using CollectiveMainLoop = cutlass::gemm::collective::CollectiveMma<
        DispatchPolicy, TileShape, ElementA,
        cutlass::gemm::TagToStrideA_t<LayoutA>, ElementB,
        cutlass::gemm::TagToStrideB_t<LayoutB>, TiledMma, GmemTiledCopyA,
        SmemLayoutAtomA, SmemCopyAtomA, identity, GmemTiledCopyB,
        SmemLayoutAtomB, SmemCopyAtomB, identity>;
...
void GemmCutlass(const int M, const int N, const int K, const float alpha,
                 const cutlass::half_t* A, const uint64_t lda,
                 const cutlass::half_t* B, const uint64_t ldb, const float beta,
                 float* C, const uint64_t ldc)
{
    ...
    using Config =
        GemmConfiguration<cutlass::arch::OpClassTensorOp, cutlass::arch::Sm80,
                          cutlass::half_t, LayoutA, cutlass::half_t, LayoutB,
                          float, LayoutC, float>;

    using GemmKernel =
        cutlass::gemm::kernel::GemmUniversal<Shape<int, int, int>,
    ...
    Gemm::Arguments arguments{cutlass::gemm::GemmUniversalMode::kGemm,
                              {M, N, K},
                              {A, dA, B, dB},
                              {{alpha, beta}, C, dC, C, dC}};
...
int main()
{
    ...
    cutlass::HostTensor<ElementA, cutlass::layout::RowMajor> A(
        cutlass::MatrixCoord(M, K));
    cutlass::HostTensor<ElementB, cutlass::layout::ColumnMajor> B(
        cutlass::MatrixCoord(K, N));
    cutlass::HostTensor<ElementC, cutlass::layout::RowMajor> C_cutlass(
        cutlass::MatrixCoord(M, N));
    cutlass::HostTensor<ElementC, cutlass::layout::RowMajor> C_reference(
        cutlass::MatrixCoord(M, N));
    ...
    cutlass::reference::device::TensorFillRandomGaussian(
        A.device_view(), seed * 2, mean, stddev, bits_less_than_one);

    cutlass::reference::device::TensorFillRandomGaussian(
        B.device_view(), seed * 3, mean, stddev, bits_less_than_one);

    cutlass::reference::device::TensorFillRandomGaussian(
        C_cutlass.device_view(), seed * 5, mean, stddev, bits_less_than_one);
    ...
    GemmCutlass(M, N, K, 1.0f, A.device_data(), A.stride(0), B.device_data(),
                B.stride(0), 0.0f, C_cutlass.device_data(),
                C_cutlass.stride(0));

    cudaDeviceSynchronize();

    GemmReference(M, N, K, 1.0f, A.device_data(), B.device_data(), 0.0f,
    ...
    if (cutlass::reference::host::TensorRelativelyEquals(
            C_cutlass.host_view(), C_reference.host_view(), ElementC(1e-2),
            ElementC(1e-5)))
    {
        std::cout << "Success" << std::endl;
    }
