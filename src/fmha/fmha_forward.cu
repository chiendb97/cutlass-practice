...
{
    auto cmd = cutlass::CommandLine(argc, argv);
    ...
    int batchSize, seqLength, dimSize, kHeadSize, nStreams, iterations;
    bool refCheck, printValues;

    cmd.get_cmd_line_argument("batch-size", batchSize, 4);
    cmd.get_cmd_line_argument("seq-length", seqLength, 512);
    ...
    {
        std::cout << "Number of cuda streams must be less than or equal to batch size";
        exit(-1);
    }
    ...
    if (kHeadSize == 64)
    {
        runFmhaForward<cutlass::half_t, 64>(seqLength, seqLength, numHeads, batchSize, iterations, refCheck, printValues, nStreams);
    }
    else if (kHeadSize == 128)
    {
        runFmhaForward<cutlass::half_t, 128>(seqLength, seqLength, numHeads, batchSize, iterations, refCheck, printValues, nStreams);
    }
    else if (kHeadSize == 256)
    {
        runFmhaForward<cutlass::half_t, 256>(seqLength, seqLength, numHeads, batchSize, iterations, refCheck, printValues, nStreams);
    }
    else
    {
