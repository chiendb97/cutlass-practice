                                                                                        ...
                                                                                        typename EVT::Params const epi_params)
{
    ...
    Tensor cD = make_coord_tensor(cD_mn.layout());         // (CTA_M, CTA_N) -- make_counting_tensor pre-CUTLASS v4
             ...
             typename EVT::Params& epi_params, cudaStream_t stream = 0)
{
    ...
    // Define the MMA. EVT accumulates in float; ss_op_selector picks F32F16F16 for half_t
    // inputs and F32BF16BF16 for bfloat16_t inputs.
    auto tiled_mma =
        make_tiled_mma(GMMA::ss_op_selector<TA, TB, float, Shape<_64, _64, _16>, GMMA::Major::MN, GMMA::Major::MN>(), AtomLayoutMNK{});
             ...
             typename EVT::Params& epi_params, cudaStream_t stream = 0)
{
    ...
    // inputs and F32BF16BF16 for bfloat16_t inputs.
    auto tiled_mma =
        make_tiled_mma(GMMA::ss_op_selector<TA, TB, float, Shape<_64, _64, _16>, GMMA::Major::K, GMMA::Major::K>(), AtomLayoutMNK{});
...
// Self-contained entry point for PyTorch bindings. Builds the linear-combination EVT
// (D = alpha * op(A) @ op(B) + beta * C) internally so callers don't construct the
// fusion tree. Inputs A/B are half_t or bfloat16_t; C (source) and D (output) are float.
template <class TA, class TB>
void evt_gemm_run(char transA, char transB, int m, int n, int k, float alpha, TA const* A, int ldA, TB const* B, int ldB, float beta,
                  float* C, int ldC, float* D, int ldD, cudaStream_t stream = 0)
{
    using TC = float;
    ...
    using CustomEVT = // alpha * acc + beta * C
        Sm90EVT<Sm90Compute<cutlass::homogeneous_multiply_add, TC, EVTType, RoundStyle>, // beta * C + (alpha * acc)
    ...
    typename CustomEVT::Arguments epi_args{
        {{EVTType(beta)}}, // beta
        {},                // C
        {
            {{EVTType(alpha)}}, // alpha
    ...
    gemm<CustomEVT>(transA, transB, m, n, k, alpha, A, ldA, B, ldB, beta, C, ldC, D, ldD, epi_params, stream);
}
