"""Standalone build for the gemm_evt_C extension (this kernel's binding only).

    pip install -e csrc/cute/gemm/hopper/evt_gemm        # builds gemm_evt_C
...
``bench_*.py`` load the ``.so`` with ``importlib.util.find_spec("gemm_evt_C").origin`` +
...
# --- this component -----------------------------------------------------------------------
EXT_NAME = "gemm_evt_C"
SOURCES = ["gemm_evt_binding.cu"]
EXTRA_INCLUDE_DIRS: list[str] = []   # extra -I dirs, relative to this file
LIBRARIES = ["cublas"]               # gemm_evt_sm90.cuh pulls in <cublas_v2.h>
# ------------------------------------------------------------------------------------------
