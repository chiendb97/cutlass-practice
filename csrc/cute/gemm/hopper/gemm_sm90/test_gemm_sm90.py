"""Standalone correctness test for the basic TMA+WGMMA GEMM (torch.ops.cute_kernels.gemm_sm90).
...
# rather than importing it. find_spec resolves this component's gemm_sm90_C built by its setup.py.
_spec = importlib.util.find_spec("gemm_sm90_C")
assert _spec and _spec.origin, "gemm_sm90_C not built; run: pip install -e csrc/cute/gemm/hopper"
...
def test_gemm_sm90(dtype):
    ...
    out = torch.ops.cute_kernels.gemm_sm90(a, b)
