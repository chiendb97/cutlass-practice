"""Standalone build for the gemm_sm90_C extension (this kernel's binding only).
...
``bench_*.py`` load the ``.so`` with ``importlib.util.find_spec("gemm_sm90_C").origin`` +
...
# --- this component -----------------------------------------------------------------------
EXT_NAME = "gemm_sm90_C"
SOURCES = ["gemm_sm90_binding.cu"]
