                                                                                        ...
                                                                                        Beta beta)
{
    ...
    // Epilogue (unpredicated): C = alpha * acc + beta * C, computed in float and converted to
    // the output type TC. Done explicitly (instead of cute::axpby) so it works for both half_t
    // (half accumulate) and bfloat16_t (float accumulate, where TC has no implicit assignment
    // from float).
    ...
    {
        float acc = alpha_f * static_cast<float>(tCrC(i)) + beta_f * static_cast<float>(tCgC(i));
        tCgC(i) = static_cast<TC>(acc);
    }
}
             ...
             cudaStream_t stream = 0)
{
    ...
    // Define the MMA. ss_op_selector picks the right GMMA atom for the operand dtype:
    // half_t -> F16F16F16 (half accumulate), bfloat16_t -> F32BF16BF16 (float accumulate).
    using ElementAccum = std::conditional_t<std::is_same_v<TA, cutlass::half_t>, cutlass::half_t, float>;
    auto tiled_mma = make_tiled_mma(GMMA::ss_op_selector<TA, TB, ElementAccum, Shape<_64, _64, _16>, GMMA::Major::MN, GMMA::Major::MN>());
             ...
             cudaStream_t stream = 0)
{
    ...
    // half_t -> F16F16F16 (half accumulate), bfloat16_t -> F32BF16BF16 (float accumulate).
    using ElementAccum = std::conditional_t<std::is_same_v<TA, cutlass::half_t>, cutlass::half_t, float>;
    auto tiled_mma = make_tiled_mma(GMMA::ss_op_selector<TA, TB, ElementAccum, Shape<_64, _64, _16>, GMMA::Major::K, GMMA::Major::K>());
