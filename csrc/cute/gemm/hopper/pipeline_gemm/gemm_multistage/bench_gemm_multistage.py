"""Standalone benchmark for the multistage pipelined GEMM (torch.ops.cute_kernels.gemm_multistage) vs. torch.matmul.
...
def _load_ops(torch):
    ...
    spec = importlib.util.find_spec("gemm_multistage_C")
    if spec is None or spec.origin is None:
        raise ImportError("gemm_multistage_C not built; run: pip install -e csrc/cute/gemm/hopper/pipeline_gemm/gemm_multistage")
...
def benchmark(M, N, K, dtype="fp16", iters=50):
    ...
    print(f"\ngemm_multistage  M={M} N={N} K={K}  dtype={dtype}  (C = A @ B^T)")
    row("torch.matmul", lambda: torch.matmul(a, b.t()))
    row("gemm_multistage", lambda: torch.ops.cute_kernels.gemm_multistage(a, b), check=True)


def main():
    p = argparse.ArgumentParser(description="Benchmark torch.ops.cute_kernels.gemm_multistage")
