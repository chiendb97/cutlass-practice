"""Standalone build for the gemm_multistage_C extension (this kernel's binding only).

    pip install -e csrc/cute/gemm/hopper/pipeline_gemm/gemm_multistage   # builds gemm_multistage_C
...
``bench_*.py`` load the ``.so`` with ``importlib.util.find_spec("gemm_multistage_C").origin`` +
...
# --- this component -----------------------------------------------------------------------
EXT_NAME = "gemm_multistage_C"
SOURCES = ["gemm_multistage_binding.cu"]
EXTRA_INCLUDE_DIRS: list[str] = []   # extra -I dirs, relative to this file
