...
namespace cute_gemm_multistage
{
...
struct SharedStorage
{
    ...
    union
    {
        ...
        cutlass::array_aligned<ElementC, cosize_v<SmemLayoutC>> smemC;
    };
...
template <class ProblemShape, class CtaTiler, class TA, class SmemLayoutA, class TmaA, class TB, class SmemLayoutB, class TmaB, class TC,
          class SmemLayoutC, class TmaC, class TiledMma, class Alpha, class Beta>
__global__ static __launch_bounds__(decltype(size(TiledMma{}))::value) void gemm_device(ProblemShape shape_MNK, CtaTiler cta_tiler,
                                                                                        ...
                                                                                        TC* C, CUTLASS_GRID_CONSTANT TmaC const tma_c,
                                                                                        TiledMma mma, Alpha alpha, Beta beta)
{
    ...
    // Tensor gC =
    //     local_tile(mC, cta_tiler, cta_coord, Step<_1, _1, X>{}); // (bM, bN)
    ...
    using SmemCopyAtomC = Copy_Atom<SM90_U32x4_STSM_N, TC>;

    Tensor sC = make_tensor(make_smem_ptr(smem.smemC.data()), SmemLayoutC{}); // (bM, bN)
    ...
    Tensor taacCsC = smem_thr_copy_C.partition_D(sC);
    copy(smem_tiled_copy_C, taccCrC, taacCsC);
    cutlass::arch::fence_view_async_shared(); // ensure smem writes are visible to TMA

    // NumThreads == size(mma)
    cutlass::arch::NamedBarrier::arrive(size(mma) + cutlass::NumThreadsPerWarp, cutlass::arch::ReservedNamedBarriers::EpilogueBarrier);
    ...
    auto block_tma_c = tma_c.get_slice(_0{});  // CTA slice
    Tensor tCgC = block_tma_c.partition_D(gC); // (TMA, TMA_M, TMA_K) ???
    Tensor tCsC = block_tma_c.partition_S(sC); // (TMA, TMA_M, TMA_K) ???
    ...
    if (warp_idx == 0)
    {
        ...
        cutlass::arch::NamedBarrier::sync(size(mma) + cutlass::NumThreadsPerWarp, cutlass::arch::ReservedNamedBarriers::EpilogueBarrier);
    }

    if (warp_idx == 0 && lane_predicate)
    {
        cute::copy(tma_c, tCsC, tCgC);
        // tma_c.arrive();
    }
             ...
             cudaStream_t stream = 0)
{
    ...
    // Define the MMA. ss_op_selector picks the GMMA atom for the operand dtype
    // (half_t -> half accumulate, bfloat16_t -> float accumulate).
    using ElementAccum = std::conditional_t<std::is_same_v<TA, cutlass::half_t>, cutlass::half_t, float>;
    auto tiled_mma =
        make_tiled_mma(GMMA::ss_op_selector<TA, TB, ElementAccum, Shape<_64, _64, _16>, GMMA::Major::MN, GMMA::Major::MN>(), AtomLayoutMNK{});
    ...
    auto tmaC = make_tma_copy(SM90_TMA_STORE{}, mC, sC, make_shape(bM, bN), _1{});

    // Setup and Launch

    int smem_size = sizeof(SharedStorage<TA, TB, TC, decltype(sA), decltype(sB), decltype(sC)>);
    dim3 block_dim(size(tiled_mma));
    dim3 cluster_dim(1, 2, 1);
    ...
    void const* kernel_ptr = reinterpret_cast<void const*>(
        &gemm_device<decltype(prob_shape), decltype(cta_tiler), TA, decltype(sA), decltype(tmaA), TB, decltype(sB), decltype(tmaB), TC,
                     decltype(sC), decltype(tmaC), decltype(tiled_mma), decltype(alpha), decltype(beta)>);

    CUTE_CHECK_ERROR(cudaFuncSetAttribute(kernel_ptr, cudaFuncAttributeMaxDynamicSharedMemorySize, smem_size));

    // Kernel launch
    cutlass::Status status =
        cutlass::launch_kernel_on_cluster(params, kernel_ptr, prob_shape, cta_tiler, A, tmaA, B, tmaB, C, tmaC, tiled_mma, alpha, beta);
             ...
             cudaStream_t stream = 0)
{
    ...
    using ElementAccum = std::conditional_t<std::is_same_v<TA, cutlass::half_t>, cutlass::half_t, float>;
    auto tiled_mma =
        make_tiled_mma(GMMA::ss_op_selector<TA, TB, ElementAccum, Shape<_64, _64, _16>, GMMA::Major::K, GMMA::Major::K>(), AtomLayoutMNK{});
