"""Standalone correctness test for the multistage pipelined GEMM (torch.ops.cute_kernels.gemm_multistage).
...
# rather than importing it. find_spec resolves this component's gemm_multistage_C built by its setup.py.
_spec = importlib.util.find_spec("gemm_multistage_C")
assert _spec and _spec.origin, "gemm_multistage_C not built; run: pip install -e csrc/cute/gemm/hopper/pipeline_gemm/gemm_multistage"
...
def test_gemm_multistage(dtype):
    ...
    out = torch.ops.cute_kernels.gemm_multistage(a, b)
