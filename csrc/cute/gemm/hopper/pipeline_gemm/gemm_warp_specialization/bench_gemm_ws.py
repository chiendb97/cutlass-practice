"""Standalone benchmark for the warp-specialized GEMM (torch.ops.cute_kernels.gemm_ws) vs. torch.matmul.
...
co-located modal_app.py.
    ...
    python csrc/cute/gemm/hopper/pipeline_gemm/gemm_warp_specialization/bench_gemm_ws.py --M 8192 --N 8192 --K 8192
"""
...
def _load_ops(torch):
    ...
    spec = importlib.util.find_spec("gemm_ws_C")
    if spec is None or spec.origin is None:
        raise ImportError("gemm_ws_C not built; run: pip install -e csrc/cute/gemm/hopper/pipeline_gemm/gemm_warp_specialization")
...
def benchmark(M, N, K, dtype="fp16", iters=50):
    ...
    print(f"\ngemm_ws  M={M} N={N} K={K}  dtype={dtype}  (C = A @ B^T)")
    row("torch.matmul", lambda: torch.matmul(a, b.t()))
    row("gemm_ws", lambda: torch.ops.cute_kernels.gemm_ws(a, b), check=True)


def main():
    p = argparse.ArgumentParser(description="Benchmark torch.ops.cute_kernels.gemm_ws")
