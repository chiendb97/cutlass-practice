             ...
             cudaStream_t stream = 0)
{
    ...
    // Half accumulates in half (fp32_accum=false); bfloat16 has no bf16-accumulate GMMA atom, so
    // it must accumulate in fp32. The fp32-accum accumulators need far more registers, so the
    // large 256x256x96 / 20-warp tile (tuned for half) overflows the register file. Use it only
    // for the half path; the fp32-accum (bf16) path falls back to the smaller 12-warp tile.
    constexpr bool kFp32Accum = !std::is_same_v<TA, cutlass::half_t>;

    using Kernel_traits = std::conditional_t<
        !validation && !kFp32Accum,
        Kernel_traits<256, 256, 128 - 32, 20, 2, /*ClusterM=*/1, /*ClusterN=*/2, TA, TC, /*fp32_accum=*/false>, // fp16 perf config
        Kernel_traits<128, 256, 128, 12, 2, 1, 1, TA, TC, kFp32Accum> // fp16 validation + all bf16
        >;
