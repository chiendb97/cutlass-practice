"""Standalone build for the fmha_C extension (this kernel's binding only).

    pip install -e csrc/cute/fmha/hopper         # builds fmha_C
...
``bench_*.py`` load the ``.so`` with ``importlib.util.find_spec("fmha_C").origin`` +
...
# --- this component -----------------------------------------------------------------------
EXT_NAME = "fmha_C"
SOURCES = ["fmha_binding.cu"]
EXTRA_INCLUDE_DIRS = ["lib/gemm"]    # fmha_forward.cuh includes copy_tensor.hpp / gemm_tensor.hpp
